...
use App\FinalTestStation4a as FinalTestStation4a;
use Illuminate\Http\Request;
use Auth;
use App\Historial as Historial;
class FinalTestStation4aController extends Controller {
	...
	{
		$rows = FinalTestStation4a::SerialNumber($request->get('SerialNumber'))->paginate(30);
	...
	{
		$object= FinalTestStation4a::findOrFail($id);
		$object->delete();
		$message = $object->SerialNumber.' fue eliminado.';
		$tableName = FinalTestStation4a::TableName();
		if (Auth::user())
        {
		...
		return redirect()->route('estaciones.finalTestStation4a.index');
	}
